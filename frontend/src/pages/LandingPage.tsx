...
const featureKeys = ['auth', 'tenant', 'billing', 'invites', 'apikeys', 'oauth', 'tests', 'docker', 'react'] as const
    ...
    <div className="min-h-screen">
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-neutral-200/60 dark:border-neutral-800/60 bg-white/80 dark:bg-neutral-950/80 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            <LanguageToggle />
            <ThemePicker />
            <Link to="/login"><Button variant="ghost">{t('nav.sign_in')}</Button></Link>
            <Link to="/register"><Button>{t('nav.get_started')}</Button></Link>
          </div>
        </div>
      </header>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-16">
        ...
        <div className="max-w-6xl mx-auto px-4 py-24 text-center">
          ...
          <div className="animate-fade-in-up animate-fade-in-up-delay-3 flex flex-wrap gap-4 justify-center">
            <Link to="/register"><Button size="lg" className="bg-brand-600 hover:bg-brand-700 text-white shadow-lg shadow-brand-500/25">{t('hero.cta_start')}</Button></Link>
            <a href="#features"><Button variant="outline" size="lg">{t('hero.cta_features')}</Button></a>
          </div>
        </div>
      </section>
                ...
                >
                  ...
                  <div className="relative p-6">
                    <div className={cn('flex items-start justify-between mb-3')}>
                      ...
                      <svg
                        className={cn('size-5 text-neutral-400 transition-transform duration-300', isOpen && 'rotate-180')}
                        fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"
                      >
                        <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                      </svg>
                    </div>
                    <h3 className="font-semibold mb-2">{t(`features.${fk}.title`)}</h3>
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-bold mb-4">{t('ai.heading_line1')}<br />{t('ai.heading_line2')}</h2>
              <p className="text-neutral-600 dark:text-neutral-400 mb-8 leading-relaxed">{t('ai.body')}</p>
              <div className="grid grid-cols-2 gap-3 mb-8">
                {[1,2,3,4,5,6].map((i) => (
                  <div key={i} className="flex items-start gap-2.5">
                    ...
                    <div>
                      <div className="text-sm font-medium">{t(`ai.item${i}_title`)}</div>
                      <div className="text-xs text-neutral-500">{t(`ai.item${i}_desc`)}</div>
                    </div>
                  </div>
                ))}
              </div>
              <Link to="/register">
                <Button size="lg" className="bg-brand-600 hover:bg-brand-700 text-white shadow-lg shadow-brand-500/25">{t('ai.cta')}</Button>
              </Link>
            </div>
            <div className="relative">
              <div className="rounded-2xl border border-neutral-200/60 dark:border-neutral-800/60 bg-white dark:bg-neutral-900/50 shadow-xl overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-3 border-b border-neutral-200/60 dark:border-neutral-800/60 bg-neutral-50 dark:bg-neutral-900">
                  ...
                  <span className="text-xs text-neutral-500 font-mono ml-2">{t('ai.code_tab')}</span>
                </div>
                <div className="p-4 font-mono text-xs leading-relaxed text-neutral-600 dark:text-neutral-400 overflow-x-auto">
                  <div><span className="text-neutral-400"># </span><span className="text-brand-500">{t('ai.code_line1')}</span></div>
                  <div className="mt-2"><span className="text-neutral-400"># {t('ai.code_line2')}</span></div>
                  <div><span className="text-neutral-500">- </span>{t('ai.code_line3')}</div>
                  <div><span className="text-neutral-500">- </span>{t('ai.code_line4')}</div>
                  <div><span className="text-neutral-500">- </span>{t('ai.code_line5')}</div>
                  <div><span className="text-neutral-500">- </span>{t('ai.code_line6')}</div>
                  <div className="mt-2"><span className="text-neutral-400"># {t('ai.code_line7')}</span></div>
                  <div>{t('ai.code_line8')}</div>
                  <div>{t('ai.code_line9')}</div>
                  <div className="mt-2"><span className="text-neutral-400"># {t('ai.code_line10')}</span></div>
                  <div>{t('ai.code_line11')}</div>
                  <div>{t('ai.code_line12')}</div>
                  <div className="mt-3 text-brand-500 font-semibold">{t('ai.code_footer')}</div>
                </div>
              </div>
      ...
      <section className="relative py-24 border-t border-neutral-200/60 dark:border-neutral-800/60">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-neutral-600 dark:text-neutral-400 mb-12 max-w-lg mx-auto">{t('pricing.subtitle')}</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-2xl mx-auto">
            <div className="relative p-8 rounded-2xl border border-neutral-200/60 dark:border-neutral-800/60 bg-white dark:bg-neutral-900/50">
              <h3 className="text-lg font-semibold mb-2">{t('pricing.solo_name')}</h3>
              <div className="text-4xl font-bold mb-1">{t('pricing.solo_price')}</div>
              <p className="text-sm text-neutral-500 mb-6">{t('pricing.solo_desc')}</p>
              <ul className="text-sm space-y-3 text-left mb-8">
                {[1,2,3,4,5].map((i) => (
                  <li key={i} className="flex items-center gap-2">
                    ...
                    {t(`pricing.solo_bullet${i}`)}
                  </li>
                ))}
              </ul>
              <Button className="w-full bg-brand-600 hover:bg-brand-700 text-white shadow-lg shadow-brand-500/25">{t('pricing.solo_cta')}</Button>
            </div>
            <div className="relative p-8 rounded-2xl border-2 border-brand-500/40 dark:border-brand-500/40 bg-white dark:bg-neutral-900/50 shadow-xl shadow-brand-500/10">
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 bg-brand-600 text-white text-xs font-medium rounded-full">
                {t('pricing.team_badge')}
              </div>
              <h3 className="text-lg font-semibold mb-2">{t('pricing.team_name')}</h3>
              <div className="text-4xl font-bold mb-1">{t('pricing.team_price')}</div>
              <p className="text-sm text-neutral-500 mb-6">{t('pricing.team_desc')}</p>
              <ul className="text-sm space-y-3 text-left mb-8">
                {[1,2,3,4].map((i) => (
                  <li key={i} className="flex items-center gap-2">
                    <svg className="size-4 text-brand-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                    {t(`pricing.team_bullet${i}`)}
                  </li>
                ))}
              </ul>
              <Button className="w-full bg-brand-600 hover:bg-brand-700 text-white shadow-lg shadow-brand-500/25">{t('pricing.team_cta')}</Button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="relative py-24 border-t border-neutral-200/60 dark:border-neutral-800/60">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-neutral-600 dark:text-neutral-400 mb-8 max-w-md mx-auto">{t('cta.body')}</p>
          <div className="flex flex-wrap gap-4 justify-center">
            <Link to="/register"><Button size="lg" className="bg-brand-600 hover:bg-brand-700 text-white shadow-lg shadow-brand-500/25">{t('cta.get_started')}</Button></Link>
            <a href="#features"><Button variant="outline" size="lg">{t('cta.learn_more')}</Button></a>
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-neutral-200/60 dark:border-neutral-800/60 py-8 text-center text-sm text-neutral-500">
        <div className="max-w-6xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-4">
          <span className="font-semibold bg-gradient-to-r from-brand-500 to-brand-700 bg-clip-text text-transparent">{t('footer.brand')}</span>
