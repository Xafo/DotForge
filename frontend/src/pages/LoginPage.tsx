        ...
        <CardContent className="space-y-4">
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-white dark:bg-neutral-950 px-2 text-neutral-500">{t('login.divider')}</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder={t('login.email_placeholder')} />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">{t('login.password_label')}</label>
              <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required placeholder={t('login.password_placeholder')} />
            </div>
            <Button type="submit" className="w-full" disabled={loading}>{loading ? t('login.submitting') : t('login.submit')}</Button>
          </form>

          <p className="text-sm text-center text-neutral-600 dark:text-neutral-400">
            {t('login.no_account')} <Link to="/register" className="text-brand-600 dark:text-brand-400 hover:underline">{t('login.register_link')}</Link>
          </p>
        </CardContent>
      </Card>
    </div>
